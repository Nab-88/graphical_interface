...
int ei_main(int argc, char** argv)
{
    ...
    while (event.type != ei_ev_keydown)
        hw_event_wait_next(&event);
    ei_color_t		color3		= { 255, 255, 0, 255 };
    ei_linked_point_t	pts3[5];
    ...
    for(i = 1; i <= 4; i++) {
        ...
        pts3[i].point.x = pts3[i-1].point.x + xdiff;
        pts3[i].point.y = pts3[i-1].point.y + ydiff;
        pts3[i-1].next = &(pts3[i]);
    }
    /* End the linked list */
    pts3[i-1].next = NULL;
    ei_draw_polygon(main_window, pts3, color3, NULL);
